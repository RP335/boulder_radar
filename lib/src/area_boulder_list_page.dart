...
class AreaBoulderListPage extends StatefulWidget {
  final String areaId;
  final String areaName;

  const AreaBoulderListPage({
    Key? key,
  ...
  @override
  State<AreaBoulderListPage> createState() => _AreaBoulderListPageState();
}

// PASTE THIS ENTIRE CLASS INTO zone_boulder_list_page.dart

class _AreaBoulderListPageState extends State<AreaBoulderListPage> {
  ...
  Future<void> _fetchAndSetBoulders() async {
    try {
      // Calls the new method with areaId
      final boulders = await _fetchBouldersInArea(widget.areaId);
      if (mounted) {
        setState(() {
  ...
  Future<List<Map<String, dynamic>>> _fetchBouldersInArea(String areaId) async {
    try {
      ...
      print('Error in _fetchBouldersInArea: $e');
  ...
  // in lib/src/area_boulder_list_page.dart
      ...
      child: ListTile(
        ...
        onTap: () async {
          final result = await Navigator.of(context).push(
            MaterialPageRoute(
              // --- THIS IS THE FIX ---
              // The constructor will only need the boulderId now, as the RPC fetches the rest.
              builder: (_) => BoulderDetailPage(boulderId: id),
            ),
          );
          // If the detail page returned 'true', it means something changed.
          if (result == true && mounted) {
            // This will re-trigger the FutureBuilder and refresh the list if needed.
            setState(() {
              _fetchFuture = _fetchAndSetBoulders();
            });
          }
      ...
      children: [
        ...
        Expanded(
          child: FutureBuilder<List<Map<String, dynamic>>>(
            future: _getSavedBouldersForArea(widget.areaId),
